...
def rescale_results(results: list, vname="undefined"):
    ...
    for abbr, full_name in dbs.items():
        # Adjust path to point to DOVER directory
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-o", "--opt", type=str, default="../../DOVER/dover.yml", help="the option file"
    )
    ...
    # Fix the model weight path to be relative to DOVER directory
    if opt["test_load_path"].startswith("./"):
        opt["test_load_path"] = "../../DOVER/" + opt["test_load_path"][2:]
